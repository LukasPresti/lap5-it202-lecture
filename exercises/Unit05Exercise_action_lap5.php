<?php
require_once('database.php');
?>
<!DOCTYPE html>
<html>
<head>
  <title>Unit 5 Exercise lap5</title>
</head>
<body>
  <h1>My Golfing Team</h1>
  <h4>Name: Lukas Presti</h4>
  <h4>UCID: lap5</h4>
  <h4>Course and Section: IT-202-### Internet Applications</h4>

<?php
// show $_POST in the terminal (useful for debugging)
error_log("\$_POST " . print_r($_POST, true));

$db = getDB();

// use $_POST because the form used method="post"
$golferid = $_POST['golferid'];
$score = $_POST['score'];

// prepare an INSERT statement
$query = "INSERT INTO games (golferid, score) VALUES (?, ?)";
$stmt = $db->prepare($query);
if($stmt == false) {
  echo "ERROR: prepare(): " . $db->errno . ' ' . $db->error;
  exit;
}

// bind parameters: both integers
$stmt->bind_param("ii", $golferid, $score);

// execute
$result = $stmt->execute();

// close statement and connection
$stmt->close();
$db->close();

if ($result) {
   echo "<h2>New game's score successfully added</h2>\n";
} else {
   echo "<h2>Sorry, there was a problem adding that game's score</h2>\n";
}

date_default_timezone_set("America/New_York");
echo "The date and time is " . date("D M j h:i:sa T Y");
?>
</body>
</html>
